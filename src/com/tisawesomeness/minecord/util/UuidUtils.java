...
public final class UuidUtils {
    ...
    private static Optional<UUID> fromStringIntArray(@NonNull CharSequence str) {
        ...
        OptionalInt oa = MathUtils.safeParseInt(arrMatcher.group(2));
        if (!oa.isPresent()) {
            return Optional.empty();
        }
        int a = oa.getAsInt();
        OptionalInt ob = MathUtils.safeParseInt(arrMatcher.group(3));
        if (!ob.isPresent()) {
            return Optional.empty();
        }
        int b = ob.getAsInt();
        OptionalInt oc = MathUtils.safeParseInt(arrMatcher.group(4));
        if (!oc.isPresent()) {
            return Optional.empty();
        }
        int c = oc.getAsInt();
        OptionalInt od = MathUtils.safeParseInt(arrMatcher.group(5));
        if (!od.isPresent()) {
            return Optional.empty();
        }
        int d = od.getAsInt();
    ...
    private static Optional<UUID> fromMostLeast(@NonNull String str) {
        ...
        if (mostLeastMatcher.matches()) {
            OptionalLong msbOpt = MathUtils.safeParseLong(mostLeastMatcher.group(1));
            if (!msbOpt.isPresent()) {
                return Optional.empty();
            }
            long msb = msbOpt.getAsLong();
            OptionalLong lsbOpt = MathUtils.safeParseLong(mostLeastMatcher.group(2));
            if (!lsbOpt.isPresent()) {
                return Optional.empty();
            }
    ...
    private static Optional<UUID> fromMostLeastNBT(@NonNull String str, int commaIndex) {
        ...
        OptionalLong firstNumOpt = MathUtils.safeParseLong(firstNumStr);
        ...
        OptionalLong secondNumOpt = MathUtils.safeParseLong(secondNumStr);
    ...
    public static @NonNull UUID fromInts(int a, int b, int c, int d) {
        ...
        long msb = (((long) a) << Integer.SIZE) | MathUtils.castWithoutSignExtension(b);
        long lsb = (((long) c) << Integer.SIZE) | MathUtils.castWithoutSignExtension(d);
